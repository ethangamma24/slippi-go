...
type ActionCounts struct {
	...
	EdgeCancelCount struct {
		Success int `json:"success"`
		Slow    int `json:"slow"`
	} `json:"edgeCancelCount"`
	LCancelCount struct {
		Success int `json:"success"`
		Fail    int `json:"fail"`
	} `json:"lCancelCount"`
	...
	GroundTechCount struct {
		Away    int `json:"away"`
		In      int `json:"in"`
		Neutral int `json:"neutral"`
		Fail    int `json:"fail"`
	} `json:"groundTechCount"`
	WallTechCount struct {
		Success int `json:"success"`
		Fail    int `json:"fail"`
	} `json:"wallTechCount"`
}
...
func Compute(game nativeslippi.Game) Stats {
	lastFrame := latestFrameNumber(game.Data.Frames)
	playableFrameCount := lastFrame - nativeslippi.FirstPlayableFrame
	...
	for frameNum := nativeslippi.FirstFrame; frameNum <= lastFrame; frameNum++ {
	...
	gameComplete := game.Data.GameEnd.GameEndMethod != nativeslippi.GameEndUnresolved
...
func latestFrameNumber(frames map[int]nativeslippi.Frame) int {
	latest := 0
	for k := range frames {
		if k > latest {
			latest = k
...
func activePlayers(players []nativeslippi.Player) []nativeslippi.Player {
	out := make([]nativeslippi.Player, 0, len(players))
	for _, p := range players {
		if p.PlayerType != nativeslippi.PlayerTypeEmpty {
			out = append(out, p)
		}
	}
	return out
...
func singlesPlayerPermutations(players []nativeslippi.Player) []playerPair {
	if len(players) != 2 {
		return nil
	}
	return []playerPair{
...
func processActions(state *playerActionState, player, opponent nativeslippi.PostFrameUpdate) {
...
func processStocks(frames map[int]nativeslippi.Frame, state *playerStockState, pair playerPair, player nativeslippi.PostFrameUpdate, stocks *[]Stock) {
	currentFrame := player.FrameNumber
	prevFrame := currentFrame - 1
	var prevPlayer *nativeslippi.PostFrameUpdate
...
func processConversions(frames map[int]nativeslippi.Frame, state *playerConversionState, pair playerPair, player, opponent nativeslippi.PostFrameUpdate, conversions *[]Conversion) {
...
func processCombos(frames map[int]nativeslippi.Frame, state *playerComboState, pair playerPair, player, opponent nativeslippi.PostFrameUpdate, combos *[]Combo) {
	prevPlayer, prevOpponent := prevFrames(frames, pair, player.FrameNumber-1)
	oppState := opponent.ActionStateID
	oppDamaged := isDamaged(oppState)
	oppGrabbed := isGrabbed(oppState)
...
func processInputs(frames map[int]nativeslippi.Frame, state *playerInputState, pair playerPair, player nativeslippi.PreFrameUpdate) {
	// Must use pre.frame so prev-frame joins use the same frame source as input counting.
	currentFrame := player.FrameNumber
	prevFrame := currentFrame - 1
	if currentFrame < nativeslippi.FirstPlayableFrame {
		return
	}
...
func prevFrames(frames map[int]nativeslippi.Frame, pair playerPair, frame int) (*nativeslippi.PostFrameUpdate, *nativeslippi.PostFrameUpdate) {
	pf, ok := frames[frame]
	if !ok {
		return nil, nil
	}
...
func generateOverall(players []nativeslippi.Player, inputStates map[playerPair]*playerInputState, conversions []Conversion, playableFrameCount int, isTeams bool) []Overall {
...
func calcDamageTaken(frame nativeslippi.PostFrameUpdate, prev *nativeslippi.PostFrameUpdate) float64 {
	if prev == nil {
		return 0
	}
...
func didLoseStock(frame, prev nativeslippi.PostFrameUpdate) bool {
...
func isCompletedFrame(frameNum int, players []uint8, frame nativeslippi.Frame) bool {
...
func incrementTechs(counts *ActionCounts, anim uint16, player, opponent nativeslippi.PostFrameUpdate) {
